<%@ Page Title="" Language="C#" MasterPageFile="~/Site1.Master" AutoEventWireup="true" CodeBehind="ListproductByCate.aspx.cs" Inherits="Bakery_Nhom7.ListproductByCate" %>
<asp:Content ID="Content1" ContentPlaceHolderID="product" runat="server">
    <asp:DataList ID="DataList1" runat="server" DataKeyField="MaSanPham" DataSourceID="SqlDataSource1" RepeatColumns="3" Width="647px">
        <ItemTemplate>
     
        <a href="Detail.aspx?MaSanPham=<%# Eval("MaSanPham") %>">
             <asp:Image ID="hinhanh" runat="server" ImageUrl='<%# Eval("hinhanh") %>' Width="250px" Height="200px" /></a>
        <br /><br />
        ...
        <a href='ShoppingCart.aspx?MaSanPham=<%# Eval("MaSanPham") %>&name=<%# Eval("TenSanPham")
                       %>&image=<%#Eval("hinhanh")
                       %>&price=<%# Eval("GiaGoc") %>&action=add' class="btnMuahang">Mua hàng</a>
         </ItemTemplate>  
    </asp:DataList>
